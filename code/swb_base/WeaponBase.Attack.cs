...
public partial class WeaponBase
{
	...
	public virtual void Attack( ClipInfo clipInfo, bool isPrimary )
	{
		...
		{
			if ( Game.IsServer )
				_ = AsyncBoltBack( GetRealRPM( clipInfo.RPM ), General.BoltBackAnim, General.BoltBackTime,
						General.BoltBackEjectDelay, clipInfo.BulletEjectParticle, true );
		}
	...
	public virtual void ShootBullet( float spread, float force, float damage, float bulletSize, bool isPrimary )
	{
		...
		if ( isPrimary )
		{
			Primary.BulletType.FireSV( this, player.EyePosition, endPos, forward, spread, force, damage, bulletSize,
					isPrimary );
		}
		else
		{
			Secondary.BulletType.FireSV( this, player.EyePosition, endPos, forward, spread, force, damage, bulletSize,
					isPrimary );
		}
	...
	/// </summary>
	[ClientRpc]
	public virtual void ShootClientBullet( Vector3 startPos, Vector3 endPos, Vector3 forward, float spread, float force,
	                                       float damage, float bulletSize, bool isPrimary )
	{
		if ( Owner is not PlayerBase player ) return;

		if ( isPrimary )
		{
			Primary.BulletType.FireCL( this, player.EyePosition, endPos, forward, spread, force, damage, bulletSize,
					isPrimary );
		}
		else
		{
			Secondary.BulletType.FireCL( this, player.EyePosition, endPos, forward, spread, force, damage, bulletSize,
					isPrimary );
		}
	}
	...
	/// </summary>
	protected async Task AsyncBoltBack( float boltBackDelay, string boltBackAnim, float boltBackTime,
	                                    float boltBackEjectDelay, string bulletEjectParticle, bool force = false )
	{
		var player = Owner as PlayerBase;
