...
public class ViewModelBaseEx : BaseViewModel
{
	...
	public ViewModelBaseEx( WeaponBase weapon )
	{
		this.weapon = weapon;
	}
	...
	private void HandleWalkAnimation( ref CameraSetup camSetup )
	{
		...
		targetVectorRot -= new Vector3( (Math.Clamp( MathF.Cos( breatheTime ), -0.3f, 0.3f ) * 2.0f) * walkSpeed / maxWalkSpeed,
				(-MathF.Cos( breatheTime / 2.0f ) * 1.2f) * walkSpeed                                / maxWalkSpeed - yaw * 1.5f, roll );
	}
	...
	private void HandleSwayAnimation( ref CameraSetup camSetup )
	{
		...
		angDif = new Angles( angDif.pitch,
				MathX.RadianToDegree( MathF.Atan2( MathF.Sin( MathX.DegreeToRadian( angDif.yaw ) ), MathF.Cos( MathX.DegreeToRadian( angDif.yaw ) ) ) ), 0 );
